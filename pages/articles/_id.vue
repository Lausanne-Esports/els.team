<template>
  <article class="single">
    <header class="d-flex align-items-end">
      <div class="image-head" v-lazy:background-image="thumbnail" />
      <div class="container">
        <p class="category">
    ...
    <div class="action-bar d-flex align-items-center">
      <div class="container d-flex">
        ...
        <div class="lang-menu" v-if="article.translations.length > 1">
          <nuxt-link :to="frUrl" :class="{ current: lang == 'fr' }"
            >fr</nuxt-link
          >
          /
          <nuxt-link :to="enUrl" :class="{ current: lang == 'en' }"
            >en</nuxt-link
          >
        </div>
      </div>
      ...
      meta: [
        {
          hid: "og:title",
          property: "og:title",
          content: `${this.article.headline} | Lausanne Esports`
        },
        {
          hid: "og:name",
          property: "og:name",
        ...
        },
        {
          hid: "og:image",
          property: "og:image",
          content: this.article.featured_thumbnail
        },
  ...
  methods: {
    async langChanged(lang) {
      this.article = await this.$axios.$get(
        "/articles/" + this.article.id + "?lang=" + lang
      );
    }
  },
